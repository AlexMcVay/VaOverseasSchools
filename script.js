...
async function loadSchoolsData() {
    try {
        const debugDiv = document.getElementById('debug-data-status');
        if (debugDiv) debugDiv.textContent = 'Loading schools data...';
        ...
        try {
            ...
            if (debugDiv) debugDiv.textContent = `Loaded from JSON: ${data?.overseas_analysis?.schools_list?.length ?? 0} schools.`;
        } catch (fetchError) {
